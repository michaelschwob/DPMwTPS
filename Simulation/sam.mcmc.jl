####
#### An MCMC Algorithm for the Stationary Abundance Model (SAM)
####
...
function samMCMC(Y, T_ind, X, H, W, nMCMC, nburn)
    ...
    for k in ProgressBar(2:nMCMC)
        ...
        ###
        ### Update lambda_t (t = 2, ..., T-1) for each species
        ### Changes : Add transposes
        ###
        ...
        ###
        ### Update lambda_T for each species
        ###
        ...
        if k > nburn
            ...
            for j in 1:J
                tmprt = exp(transpose(X[1, :] - alpha[j]*X[1, :])*beta[:, j]) - 1
                gt_save[1, j, k-nburn] = (1+tmprt)*(lam[1, j])^(alpha[j] - 1) - 1
                for t in 2:T
                    tmprt = exp(transpose(X[t, :] - alpha[j]*X[t-1, :])*beta[:, j]) - 1
                    gt_save[t, j, k-nburn] = (1+tmprt)*(lam[t-1, j])^(alpha[j] - 1) - 1
                end
            end
        end
    end
